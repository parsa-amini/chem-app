...
class ElectronDot:
    ...
    def update_position(self):
        ...
        if self.side == 0:  # top
            base_x = cx - offset/2
            base_y = cy - r
            local_pos = (base_x + self.pos_in_side * offset, base_y)
        elif self.side == 1:  # right
            base_x = cx + r
            base_y = cy - offset/2
            local_pos = (base_x, base_y + self.pos_in_side * offset)
    ...
    def draw(self, surface, highlight=False):
        if self.paired:
            ...
            color = RED if self.connected_to else (
                YELLOW if highlight else BLUE)
            radius = 8

        pygame.draw.circle(surface, color, (int(
            self.position[0]), int(self.position[1])), radius)
        pygame.draw.circle(surface, BLACK, (int(
            self.position[0]), int(self.position[1])), radius, 1)
...
class ElementDisplay:
    ...
    def create_electrons(self):
        ...
        for side in sides:
            if pairs_needed > 0:
                ...
                self.electrons.append(ElectronDot(self, side, 1, paired=True))
                pairs_needed -= 1
                count += 2
    ...
    def draw(self, surface):
        pygame.draw.circle(
            surface, GREEN if self.selected else LIGHT_GRAY, self.center, self.radius)
...
def draw_connection(surface, dot1, dot2):
    ...
    # Draw a curved line for the bond
    # Calculate control point for the curve
    cx = (x1 + x2) / 2
    cy = (y1 + y2) / 2 - 30  # Adjust this value to change the curve amount
    ...
    for i in range(len(points)-1):
        pygame.draw.line(surface, PURPLE, points[i], points[i+1], 3)
...
def main():
    ...
    while running:
                ...
                if event.button == 1:  # Left click
                    ...
                    if clicked_dot:
                        ...
                        if dragging_dot.connected_to:
                            ...
                            connections = [
                                c for c in connections if dragging_dot not in c and other_dot not in c]

                    elif clicked_element:
                        dragging_element = clicked_element
                        dragging_element.dragging = True
                        drag_offset = (
                            dragging_element.center[0] - mx, dragging_element.center[1] - my)
                        ...
                        if sym:
                            ...
                            if not any(e.symbol == sym for e in summoned_elements):
                                summoned_elements.append(
                                    ElementDisplay(sym, valence, (new_x, new_y)))
                        ...
                        if el.is_hovered((mx, my)):
                            ...
                            for e_dot in el.electrons:
                                if e_dot.connected_to:
                                    ...
                                    other_dot.connected_to = None
                                    connections = [
                                        c for c in connections if e_dot not in c and other_dot not in c]
        ...
        if dragging_dot and drag_line_start and drag_line_end:
            # Draw a curved preview line
            x1, y1 = drag_line_start
            x2, y2 = drag_line_end
            ...
            for i in range(len(points)-1):
                pygame.draw.line(screen, RED, points[i], points[i+1], 3)
